...
def mean(town, data):
    data = data.split('\n')
    data = dict(zip(towns, [re.findall(r'\d+\.\d+', i) for i in data]))
    for x in data:
    ...
    if town not in towns:
        return -1
    print(data)
    return sum(data[town])/len(data[town])
...
def variance(town, data):
    ...
    if town not in towns:
        return -1
    middle_value = sum(data[town])/len(data[town])
    a = [(i - middle_value)**2 for i in data[town]]
    return sum(a) / len(a)
